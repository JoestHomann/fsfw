...
Estimator::Estimator(const acs::Config& cfg)
    : cfg_(cfg), B_(cfg.Bcols), q_{1,0,0,0}, omega_{0,0,0}, bias_{0,0,0}, blwn_{0,0,0}, omegaMeas_{0,0,0} {}
...
void Estimator::step(const std::array<float,4>& tauW_mNm, const std::array<float,3>& tauExt_mNm) {
  ...
  auto f = [&](const float w[3], float out[3]) {
    float Jw[3] = {Jx*w[0], Jy*w[1], Jz*w[2]};
    float cross[3] = { w[1]*Jw[2]-w[2]*Jw[1], w[2]*Jw[0]-w[0]*Jw[2], w[0]*Jw[1]-w[1]*Jw[0] };
    out[0] = (tauB[0] - cross[0]) / Jx;
    out[1] = (tauB[1] - cross[1]) / Jy;
    out[2] = (tauB[2] - cross[2]) / Jz;
  };
  float k1[3]; f(omega_, k1);
  float wmid[3] = { omega_[0] + 0.5f*Ts*k1[0], omega_[1] + 0.5f*Ts*k1[1], omega_[2] + 0.5f*Ts*k1[2] };
  float k2[3]; f(wmid, k2);
  omega_[0] += Ts * k2[0]; omega_[1] += Ts * k2[1]; omega_[2] += Ts * k2[2];

  const float wx = omega_[0], wy = omega_[1], wz = omega_[2];
  float dq[4] = { -0.5f*( q_[1]*wx + q_[2]*wy + q_[3]*wz ),
  ...
  for (int i=0;i<3;i++) bias_[i] += sigB * normalRand_();
  float meas[3] = { omega_[0] + bias_[0] + sigG*normalRand_(),
                    omega_[1] + bias_[1] + sigG*normalRand_(),
                    omega_[2] + bias_[2] + sigG*normalRand_() };

  if (cfg_.gyro.fc_blwn_hz > 0.0f) {
    float alpha = 1.0f - std::exp(-2.0f * PI_F * cfg_.gyro.fc_blwn_hz * Ts);
    for (int i=0;i<3;i++) { blwn_[i] = blwn_[i] + alpha * (meas[i] - blwn_[i]); omegaMeas_[i] = blwn_[i]; }
  } else {
...
void Estimator::reset(const float q_init[4], const float w_init[3]) {
  ...
  for (int i=0;i<3;i++) { omega_[i] = w_init[i]; bias_[i]=0; blwn_[i]=0; omegaMeas_[i]=w_init[i]; }
  quatNormalize_(q_);
}

float Estimator::normalRand_() {
  static uint32_t s = 0x12345678u;
  auto urand = [&](){ s = 1664525u*s + 1013904223u; return (s>>8) * (1.0f/16777216.0f); };
  float u1 = urand(); if (u1 < 1e-7f) u1 = 1e-7f; float u2 = urand();
  return std::sqrt(-2.0f*std::log(u1)) * std::cos(6.28318530718f*u2);
}

void Estimator::quatNormalize_(float q[4]) {
  float n = std::sqrt(q[0]*q[0]+q[1]*q[1]+q[2]*q[2]+q[3]*q[3]);
  if (n > 1e-12f) { float inv=1.0f/n; for (int i=0;i<4;i++) q[i]*=inv; } else { q[0]=1; q[1]=q[2]=q[3]=0; }
}
