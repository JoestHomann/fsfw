...
ReturnValue_t RwPusService::handleReply(const CommandMessage* reply, Command_t previousCommand,
  ...
  switch (reply->getCommand()) {
    // --- Action / Commanding framework housekeeping (only once!) ---
    case ActionMessage::STEP_SUCCESS: {
      // Intermediate progress from callee
      *isStep = true;
      return returnvalue::OK;
    }
    ...
    // --- Direct command data from DeviceHandlerBase::handleDeviceTm(...) ---
    ...
    case DeviceHandlerMessage::REPLY_RAW_REPLY: {
      ...
      if (len >= 8 && p[0] == 0xAB && p[1] == 0x10) {
        ...
        // Build + send TM (service=220 is this PUS service)
        if (tmHelper.prepareTmPacket(static_cast<uint8_t>(Subservice::TM_STATUS), app,
          ...
          // FSFW API in this version: storeAndSendTmPacket()
    ...
    case DeviceHandlerMessage::REPLY_RAW_COMMAND:
      sif::error << "RwPusService: REPLY_RAW_COMMAND" << std::endl;
      return CommandingServiceBase::EXECUTION_COMPLETE;

    default: {
      // Do not escalate unknown but harmless replies to PUS[1,6]
